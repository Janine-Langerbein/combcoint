...
where $p_i$ are the $p$-values from the individual tests. Under the null hypothesis of no cointegration, C follows a chi-squared distribution with 2k degrees of freedom.
...
```
By default, the function uses lags = 1. Optionally, the user can specify the lag length manually. For example, to apply the test with 4 lags:
```{r}
